...
export const Persistence = {
  ...
  /**
   * Get saved password for a local PostgreSQL server
   */
  getServerPassword(host: string, port: number): { user: string; password: string } | null {
    if (typeof window === "undefined") return null;
    try {
      const key = `${STORAGE_PREFIX}server_${host}_${port}`;
  ...
  /**
   * Save password for a local PostgreSQL server
   */
  ...
  /**
   * Remove saved password for a local PostgreSQL server
   */
