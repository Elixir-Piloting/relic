...
import { Database, Table, Code2, Network, PanelLeft, PanelLeftClose } from "lucide-react";
...
export function DatabaseNavbar({ connectionId }: DatabaseNavbarProps) {
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: Network,
      path: `/db/${connectionId}/visualizer`,
    },
  ];
  ...
  const isActive = (path: string) => {
    if (path === `/db/${connectionId}`) {
      return pathname === path || pathname?.startsWith(`${path}/table`);
    }
