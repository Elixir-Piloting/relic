...
export class PostgreSQLAdapter implements DatabaseAdapter {
  ...
  async executeQuery<T = any>(query: string, params?: any[]): Promise<QueryResult<T>> {
    ...
    // Check connection state using public API
    const result: PGQueryResult<T> = await this.client.query<T>(query, params);
  ...
  isConnected(): boolean {
    ...
    // Check if client is ending or has connection error using type assertion
    try {
      ...
      return clientAny.connection?.stream?.writable !== false;
    } catch {
